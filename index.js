...
const totalContributions = GAMES_JSON.reduce( 
    (counter, game) => { return counter + game.backers;},
    0
);
...
const totalRaised = GAMES_JSON.reduce( 
    (counter, game) => { return counter + game.pledged;},
    0
);
...
function filterUnfundedOnly() {
    ...
    let notMetGoal = GAMES_JSON.filter(game => game.pledged < game.goal);
...
function filterFundedOnly() {
    ...
    let metGoal = GAMES_JSON.filter(game => game.pledged >= game.goal);
...
let totalUnfunded = GAMES_JSON.filter(game => game.pledged < game.goal).length;

// create a string that explains the number of unfunded games using the ternary operator
let unfundedStr = `A total of $${totalRaised.toLocaleString("us-en")} has been raised for ${totalGames} games. 
                   Currently, ${totalUnfunded } ${totalUnfunded == 1 ? "game remains" : "games remain"} 
